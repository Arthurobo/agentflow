...
- **Hid the terminal scrollbar.** Once scrolling worked, xterm's viewport
  scrollbar showed on the phone; it is now hidden for both engines. Scrolling
  stays fully functional, just without the visible bar.

## [0.5.3]

### Fixed

- **Touch-scroll in new sessions (both engines).** Dragging to scroll now works
  in Claude and OpenCode, including brand-new sessions. The terminal drives the
  scroll itself for each screen — xterm's own scrollback for Claude's normal
  buffer, and an SGR mouse-wheel report for OpenCode's alternate screen — from a
  non-passive touch handler that stops the browser hijacking the drag
  (pull-to-refresh / overscroll). Previously a fresh session couldn't be
  scrolled by finger: Claude relied on native panning that the fixed-height pane
  made unreliable, and the alternate-screen bridge wasn't driving OpenCode.
