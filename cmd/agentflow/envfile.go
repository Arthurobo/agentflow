...
# Directory for agentflow's other state (Tailscale node, admin socket).
...
# tailscale: serve a URL for your phone through your own Tailscale account.
# off: local only.
# AF_REMOTE=tailscale
...
# System Tailscale only: loopback address it forwards public requests to.
# Default: a free port. Never the same port as AF_ADDR.
# AF_TUNNEL_ADDR=
...
# --- account -----------------------------------------------------------------
# Service that emails this machine's link and lists it on the dashboard
# (sign in with agentflow account login).
